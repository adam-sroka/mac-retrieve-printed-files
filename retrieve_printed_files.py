...
def does_field_name_match(field_name, current_index, control_file_bytes):
    ...
    does_other_field_name_continue = current_index >= 3 and control_file_bytes[current_index-2] != 0
    field_name_runs_over_end = current_index + len(field_name_bytes) > len(control_file_bytes)
    if does_other_field_name_continue or field_name_runs_over_end:
        return False
    return field_name_bytes == control_file_bytes[current_index:current_index + len(field_name_bytes)]
    ...
    for index in range(len(decoded_control_file)):
        for field_name, field_type in control_file_field_types.items():
            if does_field_name_match(field_name, index, decoded_control_file):
                if field_type == 'str':
                    decoded_control_file_fields[field_name] = decode_string(index + len(field_name), decoded_control_file)
                elif field_type == 'int':
                    decoded_control_file_fields[field_name] = decode_int(index + len(field_name), decoded_control_file)
    # TODO handle more timestamp fields than time-at-creation
    if convert_timestamp:
        decoded_control_file_fields['time-at-creation'] = datetime.fromtimestamp(decoded_control_file_fields['time-at-creation']).strftime('%Y-%m-%d_%H:%M:%S')
...
def proceed_only_if_running_as_root():
    ...
    if not is_running_as_root:
        print('ERROR: please run this file as "sudo python retrieve_printed_files.py", type your password, and press enter.')
        exit()
...
if __name__ == "__main__":
    ...
    retrieved_control_file_names = [file_name for file_name in retrieved_files_names if file_name[0] == 'c']
    retrieved_data_file_names = [file_name for file_name in retrieved_files_names if file_name[0] == 'd']
    
    print(f'Found {len(retrieved_data_file_names)} previously printed files out of {len(retrieved_control_file_names)} files ever printed.')
    print('Retrieving files...')

    log_file_path = retrieved_files_directory + '/all_retrieved_files_data.csv'
    with open(log_file_path, 'w') as log_file:
        log_file.write('job-id,job-originating-user-name,time-at-creation,job-name,file-available\n')  # TODO generalise
        for file_name in retrieved_control_file_names:
            ...
            with open(control_file_path, 'rb') as control_file:
                decoded_fields = decode_control_file(control_file, CONTROL_FILE_FIELD_TYPES, convert_timestamp=True)
            log_file_line = stringify_decoded_fields(decoded_fields, CONTROL_FILE_FIELD_TYPES)
            corresponding_data_file_name = 'd' + file_name[1:] + '-001'
            is_data_file_available = corresponding_data_file_name in retrieved_data_file_names
            if is_data_file_available:
                log_file_line += 'yes\n'
                data_file_path = retrieved_files_directory + '/' + corresponding_data_file_name
                data_file_corrected_path = retrieved_files_directory + '/' + str(decoded_fields['job-id']) + '_' + decoded_fields['job-name']
